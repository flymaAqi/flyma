<template>
	<view style="height: 100%">
		<canvas :style="'width:' + canvasWidth + 'px;height:' + canvasWidth + 'px; position:relative'" type="2d"
			id="myCanvas">
	...
	export default {
		...
		methods: {
			...
			init(res) {
				...
				ctx.lineWidth = lineWidth;
				ctx.arc(0, 0, circle_r - 10, (startDegree * Math.PI) / 180 - 0.5 * Math.PI, (percent * Math.PI) / 180 + (
					startDegree * Math.PI) / 180 - 0.5 * Math.PI, false);
				ctx.stroke();
				ctx.closePath();
			}
		},
